...
module eg2c_integrated_top #(
    ...
    output wire signed [DATA_W-1:0]                                   threshold_o,
    output wire [31:0]                                                adapt_sample_count_o,
    output wire [31:0]                                                adapt_ignored_sample_count_o,
    output wire [7:0]                                                 adapt_selected_interval_o,
    output wire [ADAPT_INTERVAL_COUNT*ADAPT_COUNTER_W-1:0]            adapt_histogram_snapshot_o
);
    ...
    wire adapt_count_valid = !adapt_enable_i || (adapt_score_count_i <= ADAPT_SCORE_COUNT);
    wire all_work_done = (converter_done_q || selected_converter_done) &&
                         (!adapt_enable_i || adapt_done_seen_q || adapt_done);

    assign threshold_o = adapt_enable_i ? adapt_threshold : threshold_i;
    ...
    initial begin
        ...
        if (ADAPT_SCORE_COUNT < 0) begin
            $fatal(1, "eg2c_integrated_top ADAPT_SCORE_COUNT must be non-negative");
        end
    end
    ...
    function signed [DATA_W-1:0] get_adapt_score;
        ...
        begin
            ...
            get_adapt_score = adapt_scores_i[flat_index +: DATA_W];
        end
    endfunction
    ...
    eg2c_adapt_engine #(
        ...
        .initial_threshold_i(adapt_initial_threshold_i),
        .interval_bounds_i(adapt_interval_bounds_i),
        .threshold_o(adapt_threshold),
        .busy_o(),
        .done_o(adapt_done),
        .sample_count_o(adapt_sample_count_o),
        .ignored_sample_count_o(adapt_ignored_sample_count_o),
        .selected_interval_o(adapt_selected_interval_o),
        .histogram_o(),
        .histogram_snapshot_o(adapt_histogram_snapshot_o)
    );
            ...
            case (state_q)
                STATE_IDLE: begin
                    ...
                    if (start_i) begin
                        ...
                        precise_en_o            <= branch_precise_en;
                        converter_done_q        <= 1'b0;
                        adapt_done_seen_q       <= !adapt_enable_i;
                        adapt_update_sent_q     <= 1'b0;
                        adapt_score_idx_q       <= 32'd0;
                        selected_act_o          <= {OUT_WIDTH{1'b0}};
                        error_o                 <= !adapt_count_valid;
                        cycle_count_o           <= 32'd0;
                        converter_cycle_count_o <= 32'd0;
                        converter_op_count_o    <= 8'd0;
                        sparse_skipped_count_o  <= 32'd0;
                        adapt_done_o            <= 1'b0;
                        busy_o                  <= 1'b1;
                        state_q                 <= adapt_count_valid ? STATE_START : STATE_DONE;
                    end
                end
                ...
                STATE_RUN: begin
                    ...
                    if (adapt_enable_i && !adapt_done_seen_q) begin
                        if (!adapt_update_sent_q) begin
                            if (adapt_score_count_i == 32'd0) begin
                                adapt_update_q      <= 1'b1;
                                adapt_update_sent_q <= 1'b1;
                            end else if (adapt_score_idx_q < adapt_score_count_i) begin
                                adapt_score_q       <= get_adapt_score(adapt_score_idx_q);
                                adapt_score_valid_q <= 1'b1;
                                adapt_score_idx_q   <= adapt_score_idx_q + 32'd1;
                                if (adapt_score_idx_q == adapt_score_count_i - 32'd1) begin
                                    adapt_update_q      <= 1'b1;
                                    adapt_update_sent_q <= 1'b1;
                                end
                            end
